...
PRAKTIKANT_CLIENT_SECRET = (
    "3S87FstudnXLdngJOR9D19gtZPzBSkyvAqqqp21ZLB7cagH6uQNavI76mEK9DkHw"
)
...
async def get_auth0_token(
    ...
    async with httpx.AsyncClient() as http:
        ...
        print(
            f"[{client_name}] Client ID: {client_id[:4]}...{client_id[-4:]}, Audience: {API_AUDIENCE}"
        )
        try:
            ...
            if token:
                ...
                print(
                    f"[{client_name}] ❌ Fehler: Kein Access Token in der Antwort gefunden."
                )
                print(f"[{client_name}]   Antwort: {data}")
                return None
        except httpx.HTTPStatusError as e:
            print(
                f"[{client_name}] ❌ Fehler bei der Token-Anfrage (HTTP {e.response.status_code}):"
            )
            try:
                error_details = e.response.json()
                print(
                    f"[{client_name}]   Auth0 Fehlermeldung: {error_details.get('error_description', e.response.text)}"
                )
            except Exception:
...
async def test_tool_call(
    ...
    try:
        ...
        if not expected_success:
            print(
                f"[{client_name}] ⚠️ WARNUNG: Erfolg war nicht erwartet für '{tool_name}'. Überprüfe die Scopes!"
            )
    except McpError as e:  # Korrigierter Exception-Typ
        if expected_success:
            print(
                f"[{client_name}] ❌ FEHLER (MCP): '{tool_name}' fehlgeschlagen, obwohl Erfolg erwartet wurde: {e.message} (Code: {e.code}, Typ: {e.type})"
            )
        else:
            print(
                f"[{client_name}] ✅ ERWARTETER FEHLER (MCP): '{tool_name}' korrekt fehlgeschlagen: {e.message} (Code: {e.code}, Typ: {e.type})"
            )
            ...
            if (
                ...
                print(
                    f"[{client_name}]   Dies deutet korrekt auf fehlende Berechtigungen hin."
                )
            else:
                print(
                    f"[{client_name}]   Unerwarteter MCP Fehlercode/Typ. Überprüfe Server Logs. Details: {e}"
                )
    except Exception as e:
        print(
            f"[{client_name}] ❌ FEHLER (Allgemein): Unerwarteter Fehler bei Aufruf von '{tool_name}': {e}"
        )
