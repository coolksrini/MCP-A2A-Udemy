...
class FurnitureAgent:
    ...
    async def _get_auth0_access_token_internal(self) -> str | None:
        ...
        if not all(
            ...
            print(
                "FurnitureAgent: ERROR - Auth0 client credentials not fully configured."
            )
            return None
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                if "read:furniture" not in received_scopes:
                    print(
                        f"FurnitureAgent: WARNING - 'read:furniture' not in scopes: {received_scopes}"
                    )
                ...
                print(
                    f"FurnitureAgent: Obtained Auth0 token, expires at {self.token_expires_at.isoformat()}."
                )
                return self.current_auth0_token
        except httpx.HTTPStatusError as e:
            print(
                f"FurnitureAgent: HTTP error obtaining token: {e.response.status_code} - {e.response.text}"
            )
    ...
    async def initialize(self) -> bool:
        ...
        if not access_token:
            print(
                "FurnitureAgent: Initialization failed - could not obtain Auth0 token."
            )
            return False
        ...
        try:
            ...
            if not tools:
                print(
                    "FurnitureAgent: No tools found. Check server, token, and scopes."
                )
                self.current_auth0_token = None
                return False
            ...
            print(
                f"FurnitureAgent: MCP error loading tools: {e.message} (code={e.code}, type={e.type})"
            )
    ...
    async def ask(self, messages: List[BaseMessage]) -> str:
        if not self.is_initialized:
            print(
                "FurnitureAgent: ask() called before initialization. Reinitializing..."
            )
            if not await self.initialize():
                return (
                    "Error: Furniture assistant is unavailable, please try again later."
                )
        ...
        if self.mcp_client:
            ...
            if token_header != f"Bearer {access_token}":
                ...
                if not await self.initialize():
                    return (
                        "Error: Could not re-establish connection after token update."
                    )
        else:
        ...
        print(
            f"FurnitureAgent: Sending {len(messages)} messages. Last: '{messages[-1].content}'"
        )
        try:
            ...
            print(
                f"FurnitureAgent: MCP error during invocation: {e.message} (code={e.code}, type={e.type})"
            )
            if e.code in (401, 403):
            ...
            print(
                f"FurnitureAgent: Unexpected error during invocation: {type(e).__name__} - {e}"
            )
