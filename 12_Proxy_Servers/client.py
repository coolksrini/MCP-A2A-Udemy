...
)  # Oder dein lokaler Import
# Stelle sicher, dass StreamableHttpTransport korrekt importiert wird

MODERN_PROXY_URL = (
    "http://127.0.0.1:8000/mcp"  # URL zum StreamableHTTP-Endpunkt des Proxys
)
...
async def main():
    ...
    async with client:
        try:
            ...
            result_greet_parts = await client.call_tool(
                "greet_legacy", {"name": name_to_greet}
            )
            if result_greet_parts:
                print(
                    f"Response from 'greet_legacy' via Modern Proxy: {result_greet_parts[0].text}"
                )
            else:
            ...
            result_data_parts = await client.call_tool(
                "get_legacy_data", {"request_id": request_id_for_data}
            )
            if result_data_parts:
                print(
                    f"Response from 'get_legacy_data' via Modern Proxy: {result_data_parts[0].text}"
                )
            else:
            ...
            print(
                f"Is the modern_proxy_server.py (StreamableHTTP) running on {MODERN_PROXY_URL}?"
            )
            print(
                "Is the legacy_backend_server.py (SSE) running and accessible by the proxy?"
            )
