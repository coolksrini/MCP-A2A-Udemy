...
LEGACY_SSE_BACKEND_URL = (
    "http://127.0.0.1:9001/sse"  # !!! KORREKTUR: /sse statt /mcp/sse !!!
)


async def create_modern_proxy_instance() -> FastMCP | None:
    print(
        f"Attempting to create modern proxy for LEGACY SSE backend at: {LEGACY_SSE_BACKEND_URL}"
    )
    ...
    try:
        ...
        print(
            "FEHLER: SSETransport nicht gefunden. Stelle sicher, dass die Transportklassen korrekt importiert sind."
        )
        return None
    ...
    try:
        print(
            f"Calling FastMCP.from_client with SSE client for {LEGACY_SSE_BACKEND_URL}..."
        )
        ...
        if not tools_on_proxy:
            print(
                f"WARNUNG: Proxy '{modern_proxy_mcp.name}' wurde erstellt, aber keine Tools vom Legacy SSE Backend übernommen."
            )
        else:
            print(
                f"Proxy '{modern_proxy_mcp.name}' erstellt. Geproxyte Tools vom SSE Backend: {list(tools_on_proxy.keys())}"
            )
        return modern_proxy_mcp

    except Exception as e:
        print(
            f"FEHLER beim Erstellen des Proxys mit FastMCP.from_client() für SSE Backend: {e}"
        )
        print(f"Details: {type(e)}, {e.args}")
        print(
            f"Stelle sicher, dass der Legacy SSE Backend-Server unter {LEGACY_SSE_BACKEND_URL} läuft."
        )
        return None
...
def main():
    ...
    if proxy_mcp_instance:
        ...
        proxy_port = 8000
        print(
            f"Proxy instance created. Starting Modern Proxy Server ({proxy_mcp_instance.name}) on http://{proxy_host}:{proxy_port}/mcp (offering StreamableHTTP)"
        )

        proxy_mcp_instance.run(
            transport="streamable-http", host=proxy_host, port=proxy_port
        )
    else:
